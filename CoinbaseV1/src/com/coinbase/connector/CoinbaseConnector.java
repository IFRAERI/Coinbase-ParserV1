...
public class CoinbaseConnector {
	private static CoinbaseConnector instance;

    private Map<String, String> apiCredentials = new HashMap<>();

    private CoinbaseConnector() {
        loadApiCredentials("/home/aleksandr/Documents/api_key.txt");
    }

    public static CoinbaseConnector getInstance() {
        if (instance == null) {
            instance = new CoinbaseConnector();
        }
        return instance;
    }
    ...
    public List<RawPriceData> getPriceData(String currencyPair) {
        // Базовый URL для запроса данных о ценах
        String baseUrl = "/v2/prices/%s/spot"; // Форматный строковый литерал

        // Формируем конечный URL с подстановкой валютной пары
        String requestUrl = String.format(baseUrl, currencyPair);

        // Используйте существующую логику подключения к API для отправки запроса
        String response = sendRequestToCoinbase(requestUrl); // sendRequestToCoinbase - это гипотетический метод
