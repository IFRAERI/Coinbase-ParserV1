...
public class GoogleDriveConnector {
    ...
    public boolean appendToFileOnGoogleDrive(File file, String additionalContent) {
        try {
            ...
            JSONObject newData;
            // Проверка, что additionalContent не пуст и является JSON объектом
            if (additionalContent.trim().isEmpty() || !additionalContent.trim().startsWith("{")) {
                System.err.println("Additional content is not a valid JSON object.");
                return false;
            } else {
                newData = new JSONObject(additionalContent);
            }
            ...
            if (!isDataPresent(currentData, newData)) {
                ...
                FileContent mediaContent = new FileContent("text/plain", tempFile);
                file = service.files().update(file.getId(), null, mediaContent).execute();
